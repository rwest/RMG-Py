...
class NWChem:
	"""
	A base class for all QM calculations that use NWChem.

	Classes such as :class:`NWChemMol` and `NWChemTS` will inherit from this class.
	"""
	...
	# successKeys = [
	# 			   'Normal termination of NWChem'
	# 			  ]

	def testReady(self):
		if not os.path.exists(self.executablePath):
			raise Exception("Couldn't find NWChem executable at {0}. Try setting your GAUSS_EXEDIR environment variable.".format(self.executablePath))

	def geomToString(self, atomsymbols, atomcoords, outputString=''):
		atomCount = 0
	...
	def run(self):
		...
		# submits the input file to NWChem
		logfile = open(self.outputFilePath, 'w')
		process = Popen([self.executablePath, self.inputFilePath], stdout=PIPE, stderr=STDOUT)
	...
	def verifyOutputFile(self):
		...
		Returns a boolean flag that states whether a successful NWChem simulation already exists for the molecule with the 
	...
	def parse(self):
		"""
		Parses the results of the NWChem calculation, and returns a CCLibData object.
		"""
		parser = cclib.parser.NWChem(self.outputFilePath)
	...
	def writeInputFile(self, output, attempt=None, keywords=None, inputFilePath=None):#,  numProcShared=None, memory=None, checkPoint=False, bottomKeys=None):
		...
		with open(inputFilePath, 'w') as nwchemFile:
			nwchemFile.write(input_string)
			nwchemFile.write('\n')
		
class NWChemMol(QMMolecule, NWChem):
	"""
	A base Class for calculations of molecules using NWChem. 

	Inherits from both :class:`QMMolecule` and :class:`NWChem`.
	"""
	...
	def generateQMData(self):
		...
		if self.verifyOutputFile():
			...
			source = "QM NWChem result file found from previous run."
		else:
			self.createGeometry()
			success = False
	...
	def getParser(self, outputFile):
		...
		"""
		return cclib.parser.NWChem(outputFile)

class NWChemMolHF(NWChemMol):
	"""
	NWChem HF calculations for molecules
	...
	def directives(self, output):
		"""
		Adds the necessary options to the output file. These are different for HF, MP2, and DFT calculations,
		and include electronic structure method, convergence criterion, multiplicity, etc.
		"""
		
		if self.molecule.multiplicity > 1:
			output.append('scf')
			output.append('{0}'.format(self.multiplicityKeywords[self.molecule.multiplicity]))
			output.append('end')
		
##########################################################################################

class NWChemTS(QMReaction, NWChem):
	"""
	A base Class for calculations of transition states using NWChem. 

	Inherits from both :class:`QMReaction` and :class:`NWChem`.
	"""
	...
	def createInputFile(self, attempt, fromDoubleEnded=False, optEst=False):
		"""
		Using the :class:`Geometry` object, write the input file
		for the `attmept`th attempt.
	...
	def createIRCFile(self):
		...
		output.append('freq; {0}.hess ; end'.format(self.uniqueID))
		
		output.append('mepgs; xyz ; end')
		
		
		keywords = '{0} mepgs'.format(self.method)
	...
	def setCalculator(self, images):
		"""
		Set up the NWChem calculator for the Atomic Simulation Environment
		"""
		import ase
	...
	def runDouble(self, inputFilePath):
		self.testReady()
		# submits the input file to NWChem
		outputFilePath = os.path.splitext(inputFilePath)[0]+self.outputFileExtension
		logfile = open(outputFilePath, 'w')
		process = Popen([self.executablePath, inputFilePath], stdout=PIPE, stderr=STDOUT)
		for line in process.stdout:
			logfile.write(line)
		process.communicate()# necessary to wait for executable termination!
	...
	def runNEB(self):
		...
		return self.verifyNEBOutputFile()
	...
	def createNEBInputFile(self):
		...
		output.append('geometry endgeom nocenter noautosym noautoz')
		# Give ase the atom positions for each side of the reaction path
		...
		keywords = '{0} neb ignore'.format(self.method)
	...
	def verifyOutputFile(self):
		...
		with open(self.outputFilePath) as outputFile:
			for line in outputFile:
				...
				for element in self.failureKeys: #search for failure keywords
					if element in line:
						logging.error("NWChem output file contains the following error: {0}".format(element) )
	...
	def verifyOptOutputFile(self):
		...
		failureKeysFound = dict([(key, False) for key in self.failureKeys])
		successKeysFound = dict([('Normal termination of NWChem', False)])
	...
	def verifyNEBOutputFile(self):
		"""
		Check's that a qst2 output file exists and was successful.

		Returns a boolean flag that states whether a QST2 NWChem simulation that hasn't failed already exists for the molecule with the 
		given file name.
	...
	def verifyIRCOutputFile(self):
		...
		with open(self.ircOutputFilePath) as outputFile:
			for line in outputFile:
				...
				for element in self.failureKeys: #search for failure keywords
					if element in line:
						logging.error("NWChem IRC output file contains the following error: {0}".format(element) )
						return False
		...
		# Check that ALL 'success' keywords were found in the file.
		if not successKeysFound['Normal termination of NWChem']:
		...
		# This indexes the coordinate to be used from the parsing
			...
			ircParse = cclib.parser.NWChem(self.ircOutputFilePath)
	...
	def checkGeometry(self, outputFilePath, molecule):
		...
		"""
		parser = cclib.parser.NWChem(outputFilePath)
		parser.logger.setLevel(logging.ERROR) #cf. http://cclib.sourceforge.net/wiki/index.php/Using_cclib#Additional_information
		cclibData = parser.parse()
...
class NWChemTSB3LYP(NWChemTS):
	...
	def getQMMolecule(self, molecule):
		...
		and returning the corresponding QMMolecule from the child class NWChemMolB3LYP.
		"""

		qmMolecule = NWChemMolB3LYP(molecule, self.settings)
		return qmMolecule

class NWChemTSHF(NWChemTS):
	"""
	HF, with the cc-pvdz basis set, provides good transition state geometries
	at minimal computational cost.
	"""
	method = 'scf'
	basisSet = 'library cc-pvdz'
	...
	def directives(self, output):
		...
		if self.reactantGeom.molecule.multiplicity > 1:
			output.append('scf')
			output.append('{0}'.format(self.multiplicityKeywords[self.reactantGeom.molecule.multiplicity]))
			output.append('end')
